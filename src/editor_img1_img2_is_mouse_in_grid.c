...
static int is_mouse_in_grid(t_drown *data, unsigned short int img_switch, int cathetus)
{
    ...
    if (img_switch == 1 && (data->map.map_x < col_count && data->map.map_y < row_count))
        return (1);
    else if (img_switch == 2 && (data->map.param_x_to_modify < col_count && data->map.param_y_to_modify < row_count))
        return (1);
    return (0);
}
...
{
    if ((is_mouse_in_image(&data->editor.mouse, &data->editor.images, data->editor.images.img_switch) == 1) && data->editor.images.img_switch == 1)
    {
        if (is_mouse_in_grid(data, data->editor.images.img_switch, IMG1_CATHETUS) == 1)
            return (1);
    }
    else if ((is_mouse_in_image(&data->editor.mouse, &data->editor.images, data->editor.images.img_switch) == 1) && data->editor.images.img_switch == 2)
    {
